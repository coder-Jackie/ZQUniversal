...
#define BUTTON_HEIGHT       50  // 取消按钮的高
#define NORMAL_SPACE        7
#define LABEL_HEIGHT		45
#define LINE_NUM		    5

#define CancelBtnLeftMargin 10
#define CancelBtnBottomMargin 20
...
- (void)loadUIConfig {
    ...
    for (NSInteger i = 0; i < self.shareBtnTitleArray.count; i++) {
        ...
        CGFloat x = self.bgView.bounds.size.width / LINE_NUM * ( i % LINE_NUM);
        CGFloat y = LABEL_HEIGHT + (i / LINE_NUM) * LINE_HEIGHT;
        CGFloat w = self.bgView.bounds.size.width / LINE_NUM;
        //CGFloat h = 70;
        CGFloat h = 78; // item 高度
        ...
        //item.backgroundColor = ZQRandomColor;
...
- (UIView *)bgView {
    ...
    if (_bgView == nil) {
        ...
        NSInteger index;
        if (_shareBtnTitleArray.count % LINE_NUM == 0) {
            
            index = _shareBtnTitleArray.count / LINE_NUM;
        } else {
            
            index = _shareBtnTitleArray.count / LINE_NUM + 1;
        }
        // 加了30是为了下面留白.
        _bgView.frame = CGRectMake(0, ScreenHeight, ScreenWidth, 15 + CancelBtnBottomMargin + BUTTON_HEIGHT + (_protext.length == 0 ? 0 : 45) + LINE_HEIGHT * index);
    }
    return _bgView;
}
...
- (UIView *)topSheetView {
    ...
    if (_topSheetView == nil) {
        ...
        _topSheetView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(_bgView.frame), CGRectGetHeight(_bgView.frame) - BUTTON_HEIGHT - CancelBtnBottomMargin)];
        _topSheetView.backgroundColor = [UIColor whiteColor];
        //_topSheetView.backgroundColor = [UIColor redColor];
...
- (UILabel *)proLbl
{
    if (_proLbl == nil) {
        ...
        _proLbl.textColor = [UIColor colorWithHexString:@"111111"];
...
- (UIButton *)cancelBtn {
    ...
    if (_cancelBtn == nil) {
        ...
        _cancelBtn.frame = CGRectMake(CancelBtnLeftMargin, CGRectGetHeight(_bgView.frame) - BUTTON_HEIGHT - CancelBtnBottomMargin, CGRectGetWidth(_bgView.frame) - 2 * CancelBtnLeftMargin, BUTTON_HEIGHT);
        //	取消按钮
        [_cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
        //_cancelBtn.backgroundColor = [UIColor whiteColor];
        _cancelBtn.backgroundColor = [UIColor colorWithHexString:@"c3c6c9"];
