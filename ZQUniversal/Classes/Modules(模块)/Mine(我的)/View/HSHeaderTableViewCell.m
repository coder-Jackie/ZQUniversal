...
//外部model与本界面数据交互
- (void)setupDataModel:(HSHeaderCellModel *)model;
{
    HSHeaderCellModel *custom = (HSHeaderCellModel *)model;
    self.labelTitle.text = custom.text;
//    self.header.image = [UIImage imageNamed:@"MoreExpressionShops"];
    
    // 实现改模型改头像
    if ([NSString isBlank:model.headIVStr]) {
        model.headIVStr = @"ic_icon_photo";
    }
    self.header.image = [UIImage imageNamed:model.headIVStr];
    
    if (model.headImg) {
        self.header.image = model.headImg;
    }
